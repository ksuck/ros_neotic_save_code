...
class Ready(smach.State):
    ...
    def execute(self, ud):
        ...
        tts_res = text_to_speech_service("I'm ready for the receptionist task")
        if tts_res.success:
...
class WaitForGuest(smach.State):
    ...
    def execute(self, ud):
        ...
        tts_res = self.text_to_speech_service("Now i am ready to welcome guests")
...
class GreetGuest(smach.State):
    ...
    def execute(self, ud):
        ...
        tts_res = self.text_to_speech_service("Hello! Welcome to our office. I will ask you about information, please speak to the microphone on top of robot loudly!")
        if tts_res.success:
...
class AskForName(smach.State):
    ...
    def execute(self, ud):
        ...
        tts_res = self.text_to_speech_service("please start answer after the robot say listenning. May I have your name, please?")
        if not tts_res.success:
            return 'ask_failed'
        ...
        if stt_res.success:
            ...
            for name in list_of_names:
                guest_name = stt_res.text
                
                if ('lola' in guest_name or 'lora' in guest_name or 'rola' in guest_name or 'rora' in guest_name):
                    guest_name = 'laura'
                elif ('love it' in guest_name or 'lobin' in guest_name or 'login' in guest_name or 'lovin' in guest_name or 'rovin' in guest_name):
                    guest_name = 'robin'
                elif ('jef' in guest_name):
                    guest_name = 'jeffery'
                # rospy.loginfo(f"Guest's name: {guest_name}")
                # check = (guest_name == name)
                check = (name in guest_name)
                if check:
                    rospy.loginfo(f"Guest's name: {name}")
                    # rospy.loginfo("this is his name")
                    # tts_res = self.text_to_speech_service(f"{stt_res.text} is your name right?")
                    # if not tts_res.success:
                    #     return 'ask_failed'
                    # stt_res_check = self.speech_to_text_service()
                    # name_check = ("ye" in stt_res_check.text or "Ye" in stt_res_check.text)
                    # if name_check == True:
                    #     name_of_current_human = stt_res.text
                    #     return 'name_received'
                    # elif name_check == False:
                    #     return 'ask_failed'
                    # name_of_current_human = guest_name
                    name_of_current_human = name
                    tts_res = self.text_to_speech_service(f"Hello {name}")
                    if not tts_res.success:
                        return 'ask_failed'
                    return 'name_received'
                else:
...
class FavoriteDrinkCheck(smach.State):
    ...
    def execute(self, ud):
        ...
        tts_res = self.text_to_speech_service("please start answer after the robot say listenning.What is your favorite drink?")
        if not tts_res.success:
            return 'favorite_drink_failed'

        stt_res = self.speech_to_text_service()
        if stt_res.success:
            drink_name = stt_res.text
            # rospy.loginfo(f"drink's name: {drink_name}")
            for drink in list_of_drinks:
                # check = (drink_name == drink) or ('s' in drink_name and 'y' in drink_name)
                # check = (drink in drink_name) or ('s' in drink_name and 'y' in drink_name)
                if ('my' in drink_name or 'fav' in drink_name):
                    drink_name = drink_name.replace('my','')
                    drink_name = drink_name.replace('is','')
                if ('sorry' in drink_name or 'sory' in drink_name) or ('s' in drink_name and 'y' in drink_name):
                    drink_name = 'soy'
                if ('tee' in drink_name or 'tree' in drink_name or 'three' in drink_name):
                    drink_name = 'tea'
                check = (drink in drink_name)
                
                if check:
                    rospy.loginfo(f"drink's name: {drink}")
                    # rospy.loginfo(f"{} is his favv drink")
                    # tts_res = self.text_to_speech_service(f"{stt_res.text} is your favorite drink right?")
                    # if not tts_res.success:
                    #     return 'favorite_drink_failed'
                    # stt_res_check = self.speech_to_text_service()
                    # name_check = ("ye" in stt_res_check.text or "Ye" in stt_res_check.text or "ya" in stt_res_check.text)
                    # if name_check == True:
                    ...
                    # name_of_current_drink = drink_name
                    name_of_current_drink = drink
                    tts_res = self.text_to_speech_service(f"OK, your favorite drink is {drink}")
                    if not tts_res.success:
                        return 'ask_failed'
                    return 'favorite_drink_received'
                else:
...
class GuideToSeat(smach.State):
    ...
    def execute(self, ud):
        ...
        if nav_res.success:
            tts_res = self.text_to_speech_service("We have arrived, Please stay on my right until i give you instructions on where to sit")
            if not tts_res.success:
                return 'guide_failed'
    ...
    def execute(self, ud):
        ...
        if count_guest > 0:
            ...
            if not nav_res.success:
                ...
                return 'info_failed'
            tts_res = self.text_to_speech_service(f"The first guest is {prev_name} and favorite drink is {prev_drink}")
            rospy.loginfo(f"The first guest is {prev_name} and favorite drink is {prev_drink}")
            if not tts_res.success:
                return 'info_failed'
...
class DetectEmptySeat(smach.State):
    ...
    def execute(self, ud):
        ...
        if empty_seat_res.success:
            ...
            if not nav_res.success:
                return 'detect_seat_failed'
            tts_res = self.text_to_speech_service("Please take a seat at my face direction") 
            rospy.loginfo(f"Please take a seat at my face direction") 
            if not tts_res.success:
                return 'detect_seat_failed'
...
class CheckGuestOfNumber(smach.State):
    ...
    def execute(self, ud):
        ...
        if count_guest >= max_guest:
            ...
            rospy.wait_for_service('/rip/turtlebot/voice/text_to_speech')
            text_to_speech_service = rospy.ServiceProxy('/rip/turtlebot/voice/text_to_speech', TextToSpeech)
            tts_res = text_to_speech_service("I will move to reception point")
            if tts_res.success:
...
class End(smach.State):
    ...
    def execute(self, ud):
        ...
        rospy.wait_for_service('/rip/turtlebot/voice/text_to_speech')
        text_to_speech_service = rospy.ServiceProxy('/rip/turtlebot/voice/text_to_speech', TextToSpeech)
        tts_res = text_to_speech_service("I finished the mission")
        if tts_res.success:
            return 'end'
...
def main():
    ...
    with sm:
        ...
        # rospy.loginfo('state 0')
        # smach.StateMachine.add('READY', Ready(),
        #                        transitions={'ready_succeeded': 'WAIT_FOR_GUEST',
        #                                     'ready_failed': 'READY'})
        ...
        smach.StateMachine.add('FAVORITE_DRINK_CHECK', FavoriteDrinkCheck(),
                               transitions={'favorite_drink_received': 'GUIDE_TO_SEAT',
                                            'favorite_drink_failed': 'FAVORITE_DRINK_CHECK'})
